-- MI BOLICHE · USUARIOS, ROLES, SOPORTE Y CIERRE DE SEGURIDAD
...
-- Algunas instalaciones antiguas crearon `role` con el enum business_role.
-- Se normaliza a texto antes de usar los roles actuales de Mi Boliche.
alter table public.business_members
...
select
  exists (select 1 from information_schema.columns where table_schema='public' and table_name='business_members' and column_name='role') as roles_por_negocio,
